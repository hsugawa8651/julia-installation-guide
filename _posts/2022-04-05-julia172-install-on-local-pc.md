...
* 他のソフトウエアとの競合
  * 以下の説明は、Julia と競合するソフトウエアがインストールされていないことを前提とする
    ...
    * Anaconda Python パッケージ (Python 3.x に限る)がインストール済の場合は、それを、Juliaが参照するようにも設定できる。[3b. パッケージ設置の準備](#3b-パッケージ設置の準備) の設定が変わる。相談してください。
...
## 1. Julia のダウンロード
* Windows機なら、Microsoft Store からアプリを入手するのが最も簡単である。
  * Microsoft Store https://www.microsoft.com/ja-jp/store/apps/windows を閲覧する
  * Microsoft Store アプリのダウンロード・インストールの指示に従う。3a. に進む

![Julia Microsoft](https://i.gyazo.com/3e89f1a1f3a24fb861a1f8093398f711.png)

* Julia 言語のダウンロードのページを閲覧する。 https://julialang.org/downloads/
* `Current stable release` の節に進む
  * 2022年2月6日時点の最新版は version 1.7.2 である。 

![JuliaBinary](https://i.gyazo.com/5d74b5ef2075786f8246e0f768dcccc2.png)
...
## 2a. Julia のインストール Microsoft Store から入手した場合
* 省略

## 2b. Julia のインストール Intel CPU機の Windows 向け

* ダウンロードした `exe` ファイルをダブルクリックする
* Install ボタンを押して，作業を進める
  * インストール先ディレクトリは、変更しない（インストール場所をメモしておくとよい）
  * 以下のスクリーンショットは 1.6.0 をインストールしたときの記録です。 1.7.2に読み替えて下さい。
...
## 2c. Julia のインストール Intel CPU機の MacOS 向け
...
* `Julia-1.7.app` を `Applications` フォルダにドラッグする（Julia-1.7 のアイコンをマウスで選択して、Applications フォルダーに重ねる）
...
* ターミナルから Juliaを起動したい場合は、アプリケーション内の Juliaのシンボリックリンクを、例えば `/usr/local/bin/` に作っておくとよい。
  * ターミナルを起動して、`%`プロンプトに対して、以下の1行を打ち込む(`[ENTER]` は「 `ENTER` キーを押すこと」を意味する)。パスワードを求められる
  > `sudo ln -s /Applications/Julia-1.7.app/Contents/Resources/julia/bin/julia  /usr/local/bin/julia-17` [ENTER]

  * `/usr/local/bin/julia-17` で起動できるようになります
* [3a. Juliaを起動する](#3a-juliaを起動する) に進む

## 2d. Julia のインストール ARM機向け

* 筆者には、以下の経験がなく動作保証ができません
* Native binaries
  * Apple M1 Silicon機 (MacOSX 11.4 - Big Sur以上)向けに Julia v1.7.2 バイナリが公開されています。macOS ARM です (2022/4/5追記)
...
* Windows機の Microsoft Apps の Juliaは、アプリから起動する
...
* 次の文を打ち込む。全て半角文字である。大文字・小文字を区別する。以後 `[ENTER]` は「 `ENTER` キーを押すこと」を意味する。（下に表示されたテキストをcopy&pasteしてはいけません（貼り付けたときに文字化けします)。キーボードから打ち込んでください。以下、全てのテキストは同様です）
...
* `pkg>` プロンプトが出るまで待つ。
  * 進行していないようなら、リターンキーを何度か押してみて下さい。 
...
* 画面右側の `New` ボタンを押す．
* `Julia 1.7` を選んで起動する (`Julia 1.7.2` と表示される場合もあります)
  * kernelのリストに `Julia 1.7` が見つからない場合
